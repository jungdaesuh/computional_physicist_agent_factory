...
logger = logging.getLogger("factory.validation.cli")


def main() -> None:
    """CLI entry point for the validation module."""
    logger.info("main() called with args=%s", sys.argv[1:])
    parser = argparse.ArgumentParser(description="Validation CLI")
    parser.add_argument("--mock-mode", action="store_true", help="Run in mock mode")
    args = parser.parse_args()

    if args.mock_mode:
        print("Running validation in mock mode.")
    else:
        print("Running validation in live mode.")
