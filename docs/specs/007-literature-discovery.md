...
- This module is **Phase 0** of the factory: it takes a seed research question, walks the OpenAlex citation graph in a bounded traversal, ranks the resulting papers, promotes a small subset into the local **Paper Store** (PDFs + extracted evidence + BibTeX), and emits `GapCandidate` artifacts for the Gap Miner. It is the only place external bibliographic data enters the factory.
...
- Open first: `factory/literature/api.py` and `factory/literature/tests/test_literature_typical_usage.py`, then the example policy YAML in `factory/literature/config/`.
...
- Mock-mode example: `python -m factory.literature mine-gaps --seed-query "QI stellarator coil simplicity" --mock-mode`
- Runbook: `docs/runbooks/literature-discovery.md`

## LOCAL DEBUG
- Instantiate without OpenAlex network access: `LiteratureDiscovery.from_mock(fixture="qi_stellarator")` returns a pre-canned graph + ranked papers + sample `GapCandidate`s. The companion `PaperStore.mock()` returns a fixture Paper Store keyed by deterministic Work IDs.
- Live mode reads polite-pool email from env var **`OPENALEX_MAILTO`** (canonical; never `OPENALEX_EMAIL`); optional `OPENALEX_API_KEY` for higher rate limits.
- Common error signatures → recovery:
  ...
  - `PaperPromoteFailed` → OA PDF unavailable / OCR failed / extraction schema mismatch; paper stays as graph node only.
...
- Logs to inspect: `runs/<cycle-id>/cycle.jsonl` filtered by `module=literature`; per-traversal JSONL at `runs/<cycle-id>/literature/<run_id>.jsonl`; Paper Store entries at **`runs/_paper_store/<work_id>/`** (per FIX_PLAN §10).

## DEPENDENCIES
- **Hard:** Spec 002 (artifacts) — emits `GapCandidate`. That is the only inbound contract.
- **Soft:** Spec 014 (telemetry) — emits traversal + promotion events if available. Spec 013 (budget) — paper-promotion costs (OCR, extraction LLM) tracked if a budget context is provided. Spec 011 (RAG writer) — *downstream* consumer of the Paper Store; not a runtime dep. The writer uses `PaperStore.get_bibtex(work_id)` to render citations.
- **Mocks available:** `LiteratureDiscovery.from_mock(fixture=...)`; `PaperStore.mock()`; `MockOpenAlexClient` with canned `get_work` / `search_works` / `get_backward_references` / `get_forward_citations` responses; fixture Paper Store under `factory/literature/fixtures/paper_store/`.

---

## 1. Summary

The Literature Discovery module is the factory's only bridge to the external academic graph. It is bounded, read-only, and emits typed `GapCandidate` artifacts — never simulator parameters. The pipeline composes a typed OpenAlex client, a local OpenAlex graph cache, a bounded traversal engine, a ranker, a **`PaperStore`** with PDF/OCR + evidence extraction + BibTeX, and a Gap Miner that turns ranked literature into candidate research directions.
...
- `PaperStore`: persisted PDF + OCR text + extracted evidence + synthesized BibTeX for promoted papers only, exposed as a typed public class (§3).
- `GapMiner`: emits `GapCandidate` artifacts in the four supported `gap_type`s.
- Agent-facing tool surface (5 tools — see §3).
- Traversal policy YAML loader (`factory/literature/config/`).
...
> **LLM access (FIX_PLAN §25.2).** All LLM calls in this module — Gap Miner heuristic
> analysis, evidence extraction from OCR'd PDFs, paper summarization, and any future
> LLM-driven helper — go through the shared OpenRouter client with
> `model="google/gemini-3.5-flash"`. There is no Gemini-direct SDK import anywhere in
> this module; the `openai` Python package is the only SDK in play (OpenAI-compatible
> REST against `https://openrouter.ai/api/v1`). The single required env var is
> `OPENROUTER_API_KEY`. (§25 supersedes §24's single-vendor Gemini-only constraint.)
...
class OpenAccess(BaseModel):
    ...
    NOTE on `is_oa` dual representation:
      - In query strings, OpenAlex accepts the top-level filter `is_oa=true`.
...
class PaperStore:
    ...
    def promote(self, work_ids: list[OpenAlexWorkId]) -> None:
        """Promote one or more Work IDs from the graph into the Paper Store.

        Internally: fetch OA PDF, OCR, extract evidence, synthesize BibTeX,
        write atomically. Failure for any single work raises PaperPromoteFailed
        / EvidenceExtractionFailed / BibtexUnavailable with the offending
        Work ID; previously promoted entries remain intact.
        """
        ...
...
class OpenAlexClient:
    ...
    def search_works(
        self,
        query: str,
        filters: dict[str, str | int | bool],   # accepts `is_oa=True` (top-level filter form)
    ) -> list[OpenAlexWork]: ...
...
- Tables (stub): `works`, `edges` (with `edge_type ∈ {backward_ref, forward_cite, related_work}`), `traversal_runs`, `ranking_scores`.
...
literature_discovery:
  ...
  filters:
    type: article
    is_oa: true                 # top-level OpenAlex filter form
...
The `is_oa: true` line is the **top-level filter** form. The same flag appears as `open_access.is_oa` in OpenAlex Work response bodies — see `OpenAlexWork.open_access` and `OpenAccess` in §3 for the nested representation.
...
├── pdf.pdf                # OA PDF if available
├── ocr.txt                # OCR / parsed text
├── evidence.json          # Extracted-evidence schema instance (TBD)
├── bibtex.bib             # Synthesized BibTeX entry (one record per paper)
└── PROVENANCE.json        # source URL, fetch time, hashes, OCR tool, extraction model
```
...
- `source_papers` = list of OpenAlex Work IDs from the ranked set, all promoted into the Paper Store before emission.
...
- Polite-pool email from `OPENALEX_MAILTO` is appended to every request as `mailto=<email>`; optional `api_key` from `OPENALEX_API_KEY` for higher rate limits.
...
### 5.2 Bounded traversal (stub)

- TBD: priority-BFS frontier with score = recency × relevance × OA-PDF availability; ties broken by citation count.
- TBD: termination conditions — any of `max_depth`, `max_nodes`, `branch_factor`, `max_pages`, `wall_clock_s` exhausted → `TraversalBudgetExhausted` only if no useful work occurred; otherwise return partial run.
- TBD: dedup across runs by `work_id`.

### 5.3 Paper ranking (stub)
...
- TBD: rationale string per paper to support agent-facing tools and audit.

### 5.4 Paper Store promotion

- OA PDF fetcher with redirect / hash verification.
- OCR pipeline; deterministic-seeded extraction model call.
- BibTeX synthesis from the typed `OpenAlexWork` record (authors + year + title + venue + DOI when present). Failure to assemble a well-formed entry raises `BibtexUnavailable` and the paper is still promoted (PDF + evidence remain), but `has_bibtex(work_id)` returns `False`.
- Schema-validated `evidence.json` write; failure raises `EvidenceExtractionFailed` and the paper is **not** added to the Paper Store (graph node remains).
- All writes go through a staging-then-atomic-rename pattern (`runs/_paper_store/_staging/<work_id>/` → `runs/_paper_store/<work_id>/`) so partial promotions never leave a half-built directory.

### 5.5 Gap Miner (skeleton — heuristic policy TBD)

- **Input** (fixed contract): ranked papers + local subgraph topology + `ExtractedEvidence` entries read via `PaperStore.get(work_id)`.
...
- If zero candidates clear the rationale threshold for a given run, raise `GapMinerProducedNoCandidates`; the state machine treats this as a non-fatal null run and may widen the seed query.
...
| `PaperPromoteFailed(LiteratureError)` | OA PDF unavailable, OCR failure, or extractor refused | Paper remains a graph node only; not added to Paper Store; logged for human spot-check |
...
- `test_openalex_client_typed.py` — every client method returns the declared type against fixture responses; covers nested `open_access.is_oa` parsing AND top-level `is_oa=true` filter encoding.
...
- `test_ranker_mmr_diversity.py` — high-citation duplicates do not crowd the top-k.
- `test_paper_store_atomic_promote.py` — interrupted promotion leaves no half-built directory; `runs/_paper_store/<work_id>/` only exists after atomic rename.
...
- `test_openalex_mailto_env.py` — client reads `OPENALEX_MAILTO` (not `OPENALEX_EMAIL`) and refuses to start live mode without it.

**Live-mode** (`@pytest.mark.live`, gated):
- `test_live_seed_search.py` — single low-cost query against the real OpenAlex API.
- `test_live_promote_one_oa_paper.py` — fetch + OCR + extract + BibTeX-synthesize a known OA paper.
...
- Paper promotion: dominated by OCR + LLM extraction; per-paper budget enforced by spec 013 if a budget context is provided.
...
- [ ] Scaffold `factory/literature/` from the canonical module template.
- [ ] Implement `OpenAlexClient` typed wrapper with the required methods + polite-pool support; forward citations via `/works?filter=cites:<work_id>` only.
- [ ] Implement `OpenAlexGraphStore` (SQLite backing) with corruption check.
- [ ] Implement `TraversalEngine` with policy enforcement and partial-run support.
- [ ] Implement `PaperRanker` with MMR diversity and per-paper rationale.
- [ ] Implement `PaperStore` class with the public surface in §3 (`query`, `get`, `get_bibtex`, `has_bibtex`, `promote`, `mock`), backed by `runs/_paper_store/<work_id>/`.
- [ ] Implement atomic promotion (staging-then-rename) and BibTeX synthesis.
- [ ] Freeze `ExtractedEvidence` v1 schema; coordinate with spec 011.
- [ ] Implement `GapMiner` producing `GapCandidate` artifacts in all four underscored `gap_type`s (heuristic policy TBD).
- [ ] Implement the 5 agent-facing tools matching SPEC.md §6.4 verbatim.
- [ ] Author default `TraversalPolicy` YAML in `factory/literature/config/`.
- [ ] Author mock fixtures (graph snippet + canned PDF + canned evidence + canned BibTeX) under `factory/literature/fixtures/`.
- [ ] Write `factory/literature/cli.py` with all subcommands listed in ENTRY POINTS.
- [ ] Write the mock-mode tests listed in §7.
- [ ] Write the 2 live-mode tests behind `@pytest.mark.live`.
- [ ] Write `docs/runbooks/literature-discovery.md`.
- [ ] Write `factory/literature/README.md` (≤ 1 page, mock-mode example).
- [ ] Verify `mypy --strict factory/literature/` passes.
- [ ] Verify `python -m factory.literature mine-gaps --mock-mode` works on a fresh checkout.
- [ ] Confirm hard-boundary test (§7 `test_hard_boundary.py`) blocks any direct simulator-parameter emission.
- [ ] Confirm env-var test (§7 `test_openalex_mailto_env.py`) requires `OPENALEX_MAILTO` and rejects `OPENALEX_EMAIL`.
